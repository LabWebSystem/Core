...
import { api, ApiError, type Application, type LogEntry, type LogQuery, type Operation } from "./api/client";
...
const stateLabel = (app: Application) => app.registrationState === "UNREGISTERED" ? "登録解除済み" : app.reconciling ? "処理中" : app.observedState === "RUNNING" ? "稼働中" : app.observedState === "ERROR" ? "異常" : app.observedState === "UNKNOWN" ? "状態未確認" : "停止中";
...
const phaseLabel = (phase?: string) => ({ starting: "開始", source_prepare: "source準備", runtime_prepare: "実行設定", compose_up: "Compose起動", publish: "公開設定", unregister: "登録解除", volume_cleanup: "Docker volume削除", filesystem_cleanup: "作業領域削除", database_cleanup: "保存データ削除", queued: "開始待ち", running: "実行中", succeeded: "完了", failed: "失敗", cancelled: "中止" })[phase ?? ""] ?? phase;
...
export function App() {
  ...
  useEffect(() => {
    if (!selected) return;
    setLogs([]);
    return api.tailLogs(selected, logView, logView === "application" ? logService : undefined, logQuery, (entry) => {
  ...
  return <main className="app-shell">
    ...
    <section className="workbench">
      ...
      <section className="main-pane">{selected ? <AppWorkbench app={selected} operation={operation} variables={variableRows} configLoading={config.isLoading} onAction={action} onRun={run} onConfirm={(label, execute) => setPending({ label, execute })} /> : <RegisterForm busy={register.isPending} onSubmit={(input) => register.mutate(input)} />}</section>
      <aside className="activity-pane"><div className="pane-title"><span>操作とログ</span><ScrollText size={17} /></div><OperationView operation={operation} /><LogPane view={logView} service={logService} services={services} entries={logs} query={logQuery} onViewChange={(view) => { setLogView(view); setLogService(undefined); }} onServiceChange={setLogService} onQueryChange={setLogQuery} /></aside>
    </section>
...
function AppWorkbench({ app, operation, variables, configLoading, onAction, onRun, onConfirm }: { app: Application; operation?: Operation; variables: { name: string; isSecret: boolean }[]; configLoading: boolean; onAction: (kind: "start" | "stop" | "sync" | "rebuild" | "register") => void; onRun: (task: () => Promise<{ name: string }>) => void; onConfirm: (label: string, execute: () => Promise<{ name: string }>) => void }) {
  ...
  const disabled = app.reconciling || operation?.state === "queued" || operation?.state === "running";
  const unregistered = app.registrationState === "UNREGISTERED";
  const actionTitle = disabled ? "実行中の操作が完了するまで待ってください" : undefined;
  return <section className="app-workbench"><header className="app-heading"><div><span className="eyebrow">公開アプリ</span><h2>{app.subdomain}</h2><a href={publicUrl(app)} target="_blank" rel="noreferrer">{publicUrl(app)} <ExternalLink size={14} /></a></div><span className={`state-pill ${stateTone(app)}`}>{stateLabel(app)}</span></header><div className="action-strip" aria-label="アプリ操作">{unregistered ? <button disabled={disabled} title={actionTitle} onClick={() => onAction("register")}><CirclePlus size={16} />再登録</button> : <><button disabled={disabled} title={actionTitle} onClick={() => onAction("start")}><Play size={16} />開始</button><button disabled={disabled} title={actionTitle} onClick={() => onAction("stop")}><Square size={16} />停止</button><button disabled={disabled} title={actionTitle} onClick={() => onAction("sync")}><RotateCw size={16} />同期</button><button disabled={disabled} title={actionTitle} onClick={() => onAction("rebuild")}><Settings2 size={16} />再構成</button><button disabled={disabled} title={actionTitle} onClick={() => onConfirm("登録を解除する", () => api.unregister(app))}><FileCog size={16} />登録解除</button></>}<button className="danger-outline" disabled={disabled} title={actionTitle} onClick={() => onConfirm("アプリと保存データを完全に削除する", () => api.purge(app))}><Trash2 size={16} />完全削除</button></div><section className="details-grid"><div><span>リポジトリ</span><code>{app.repositoryUrl}</code></div><div><span>参照</span><code>{app.ref}</code></div><div><span>実行状態</span><strong>{observedStateLabel(app.observedState)}</strong></div><div><span>登録状態</span><strong>{registrationStateLabel(app.registrationState)}</strong></div>{app.latestError && <div><span>異常内容</span><strong>{app.latestError}</strong></div>}<div><span>最終確認</span><time>{new Date(app.observedAt).toLocaleString("ja-JP")}</time></div></section><section className="settings-section"><div className="section-heading"><div><span className="eyebrow">環境設定</span><h3>変数</h3></div><button className="primary small" disabled={disabled || unregistered} onClick={() => onConfirm("環境設定を更新する", () => api.saveConfiguration(app, Object.fromEntries(rows.filter((row) => row.value).map((row) => [row.name, { value: row.value, secret: row.isSecret }]))))}>保存</button></div>{configLoading ? <p>設定を読み込んでいます</p> : <><div className="variable-list">{rows.map((row) => <label key={row.name} className="variable-row"><code>{row.name}</code><input disabled={disabled || unregistered} type={row.isSecret ? "password" : "text"} value={row.value} placeholder={row.isSecret ? "secret は表示されません" : "値を入力"} onChange={(event) => setValues((current) => ({ ...current, [row.name]: { value: event.target.value, secret: row.isSecret } }))} /><small>{row.isSecret ? "secret" : "通常値"}</small></label>)}</div><div className="add-variable"><input disabled={disabled || unregistered} placeholder="変数名" value={newName} onChange={(e) => setNewName(e.target.value.toUpperCase())} /><input disabled={disabled || unregistered} placeholder="値" value={newValue} onChange={(e) => setNewValue(e.target.value)} /><label><input disabled={disabled || unregistered} type="checkbox" checked={newSecret} onChange={(e) => setNewSecret(e.target.checked)} />secret</label></div></>}</section></section>;
}

function OperationView({ operation }: { operation?: Operation }) { return <section className="operation" aria-live="polite"><span className="eyebrow">現在の操作</span>{operation ? <><strong>{operationLabel(operation.kind)}: {operation.state === "queued" ? "開始待ち" : operation.state === "running" ? "実行中" : operation.state === "succeeded" ? "完了" : operation.state === "cancelled" ? "中止" : "失敗"}</strong><small>{operation.name}</small>{operation.phase && <small>段階: {phaseLabel(operation.phase)}</small>}<time>{operation.createdAt ? `開始: ${new Date(operation.createdAt).toLocaleString("ja-JP")}` : "開始時刻を確認中です"}</time><p>{operation.errorMessage || operation.displayMessage || (operation.state === "running" || operation.state === "queued" ? "進捗を確認しています。" : "")}</p></> : <p>実行中の操作はありません。</p>}</section>; }

function LogPane({ view, service, services, entries, query, onViewChange, onServiceChange, onQueryChange }: { view: LogView; service?: string; services: string[]; entries: LogEntry[]; query: LogQuery; onViewChange: (view: LogView) => void; onServiceChange: (service?: string) => void; onQueryChange: (query: LogQuery) => void }) {
  const labels: Record<LogView, string> = { task: "タスクログ", application: "アプリログ", related: "関連ログ" };
  const [draft, setDraft] = useState({ limit: String(query.limit ?? 200), startAt: query.startAt?.slice(0, 16) ?? "", endAt: query.endAt?.slice(0, 16) ?? "" });
  const displayEntries = groupLogEntries(entries);
  const applyQuery = () => onQueryChange({ limit: Math.max(1, Math.min(500, Number(draft.limit) || 200)), startAt: draft.startAt ? new Date(draft.startAt).toISOString() : undefined, endAt: draft.endAt ? new Date(draft.endAt).toISOString() : undefined });
  return <section className="log-pane" aria-label="アプリ運用ログ"><div className="log-tabs" role="tablist" aria-label="ログ種別">{(Object.keys(labels) as LogView[]).map((item) => <button key={item} role="tab" aria-selected={view === item} onClick={() => onViewChange(item)}>{labels[item]}</button>)}</div><div className="log-filters"><label>件数<input type="number" min="1" max="500" value={draft.limit} onChange={(event) => setDraft({ ...draft, limit: event.target.value })} /></label><label>開始日時<input type="datetime-local" value={draft.startAt} onChange={(event) => setDraft({ ...draft, startAt: event.target.value })} /></label><label>終了日時<input type="datetime-local" value={draft.endAt} onChange={(event) => setDraft({ ...draft, endAt: event.target.value })} /></label><button onClick={applyQuery}>再読込</button></div>{view === "application" && <label className="service-select">対象コンテナ<select value={service ?? ""} onChange={(event) => onServiceChange(event.target.value || undefined)}><option value="">全てのコンテナ</option>{services.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>}<ol className="log-list" aria-live="polite">{displayEntries.length ? displayEntries.map((item) => <LogRow key={item.entry.id} item={item} view={view} />) : <li className="log-empty">{labels[view]}はまだありません。</li>}</ol></section>;
}
