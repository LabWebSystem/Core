...
function OperationView({ operation }: { operation?: Operation }) { return <section className="operation"><span className="eyebrow">現在の操作</span>{operation ? <><strong>{operation.state === "queued" ? "待機中" : operation.state === "running" ? "実行中" : operation.state === "succeeded" ? "完了" : "失敗"}</strong><small>{operation.name}</small>{operation.errorMessage && <p>{operation.errorMessage}</p>}</> : <p>まだ操作はありません。</p>}</section>; }
