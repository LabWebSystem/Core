...
func (s *Server) listApplications(w http.ResponseWriter, _ *http.Request) {
	...
	rows, err := s.DB.Query(`SELECT id, subdomain, repository_url, git_ref, compose_file, desired_state, observed_state, registration_state, latest_error, updated_at FROM applications WHERE registration_state IN ('ACTIVE','CONFIGURING','UNREGISTERED') ORDER BY subdomain`)
	if err != nil {
	...
	for rows.Next() {
		var id, sub, repo, ref, composeFile, desired, observed, state, latestError, updated string
		if err := rows.Scan(&id, &sub, &repo, &ref, &composeFile, &desired, &observed, &state, &latestError, &updated); err != nil {
...
func (s *Server) createApplication(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := tx.ExecContext(r.Context(), `INSERT INTO applications(id,subdomain,repository_url,git_ref,compose_file,created_at,updated_at) VALUES(?,?,?,?,?,datetime('now'),datetime('now'))`, id, req.Subdomain, req.RepositoryURL, req.Ref, req.ComposeFile); err != nil {
		_ = tx.Rollback()
