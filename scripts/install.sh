...
LWS_RELEASE_VERSION="${LWS_VERSION:-latest}"
TMPDIR="$(mktemp -d)"
...
download() {
  curl --http1.1 --fail --silent --show-error --location "$1" -o "$2"
}
...
if [[ "$LWS_RELEASE_VERSION" == latest ]]; then API="https://api.github.com/repos/$REPOSITORY/releases/latest"; else API="https://api.github.com/repos/$REPOSITORY/releases/tags/lws-v$LWS_RELEASE_VERSION"; fi
command -v curl >/dev/null 2>&1 || { printf 'curlが必要です。\n' >&2; exit 1; }
JSON="$TMPDIR/release.json"; download "$API" "$JSON"
...
FILE="$TMPDIR/$(basename "$ASSET")"; download "$ASSET" "$FILE"
